...
const uomMutations = {
   ...
   */
  async uomsAdd(_root, doc: IUom, { user, docModifier, models, subdomain }: IContext) {
   ...
   */
  async uomsEdit(_root, { _id, ...doc }: IUomsEdit, { user, models, subdomain }: IContext) {
   ...
   */
  async uomsRemove(_root, { uomIds }: { uomIds: string[] }, { user, models, subdomain }: IContext) {
    ...
    for (const uom of uoms) {
      await putDeleteLog(models, subdomain, { type: MODULE_NAMES.UOM, object: uom }, user);
    }
...
// moduleCheckPermission(uomMutations, 'manageAssets');
