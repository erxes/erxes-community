...
export const validRiskAssessments = async params => {
  ...
  if (await models?.RiskAssessments.findOne({ name: params.name })) {
  ...
  for (const logic of calculateLogics) {
    ...
    if (!logic.color) {
      throw new Error(`${logic.name} calculate status color should not be empty.Please select some color`);
    }
  ...
  for (const { name, value, value2, logic, color } of calculateLogics) {
    ...
    if (operator === '≈') {
      if (value < resultScore && resultScore < value2) {
        return await models.RiskAssessment.findOneAndUpdate({ _id: riskAssessmentId }, { $set: { status: name, statusColor: color } }, { new: true });
      }
    }
    if (['>', '<'].includes(operator)) {
      operator += '=';
      if (eval(resultScore + operator + value)) {
        await models.RiskAssessment.findOneAndUpdate({ _id: riskAssessmentId }, { $set: { status: name, statusColor: color } }, { new: true });
      }
    }
  }
};
