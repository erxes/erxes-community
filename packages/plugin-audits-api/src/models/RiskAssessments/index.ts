...
import { validRiskAssessments } from '../../common/utils/riskAssessments';
import {
  IRiskAssessmentField,
  PaginateField,
  IRiskAssessmentsDocument
} from '../../common/types/riskAssessment';
import { riskAssessmentsSchema } from '../definitions/riskAssessments';
...
export interface IRiskAssessmentsModel extends Model<IRiskAssessmentsDocument> {
  ...
  riskAssessmentDetail(params: { _id: string }): Promise<IRiskAssessmentsDocument>;
  riskAssesmentAdd(params: IRiskAssessmentField): Promise<IRiskAssessmentsDocument>;
...
export const loadRiskAssessments = (model: IModels, subdomain: string) => {
  class RiskAssessments {
    public static async riskAssessments(
      ...
      const list = paginate(model.RiskAssessments.aggregate([match, lookup, set, sort]), params);

      const totalCount = model.RiskAssessments.find(filter).countDocuments();
    ...
    public static async riskAssesmentAdd(params: IRiskAssessmentField) {
      try {
        await validRiskAssessments(params);
      } catch (e) {
        throw new Error(e.message);
      }

      return model.RiskAssessments.create({ ...params });
    }
    ...
    public static async riskAssesmentRemove(_ids: string[]) {
      ...
      try {
        await model.RiskConformities.deleteMany({ riskAssessmentId: { $in: _ids } });
        await model.RiksFormSubmissions.deleteMany({ riskAssessmentId: { $in: _ids } });
        await model.RiskAssessments.deleteMany({ _id: { $in: _ids } });
        return true;
      } catch (e) {
        throw new Error(e.message);
      }
    ...
    public static async riskAssessmentUpdate(params: { _id: string; doc: IRiskAssessmentField }) {
      ...
      const result = await model.RiskAssessments.findByIdAndUpdate(_id, doc);
      if (!result) {
    ...
    public static async riskAssessmentDetail(params: { _id: string }) {
      ...
      const [first] = await model.RiskAssessments.aggregate([match, lookup, unwind]);
      return first;
    }
  }
  riskAssessmentsSchema.loadClass(RiskAssessments);
