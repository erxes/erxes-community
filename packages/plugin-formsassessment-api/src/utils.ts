...
export const validItems = async params => {
  if (!params.tagIds) {
    throw new Error('Please select some tags');
  }
  if (await models?.Items.findOne({ name: params.name })) {
    throw new Error(
...
export const validateCalculateMethods = async params => {
  ...
  for (const calculateLogic of params.metrics || []) {
...
export const calculateRiskAssessment = async (models, cardId, cardType) => {
  ...
  const { metrics } = await models.RiskAssessment.findOne({
    _id: riskAssessmentId
  }).lean();

  for (const { name, value, value2, logic, color } of metrics) {
...
export const getItemSubmissions = async ({
  models,
  subdomain,
  cardId,
  cardType,
  ...
  let match: any = { assessmentId, itemId };
  ...
  const submissions = await models.AssessmentsSubmissions.aggregate([
    {
      $match: match
    },
    {
