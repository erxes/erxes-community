...
const TypesListContainer = (props: FinalProps) => {
  const { listKbcgwTypeQuery, typesEdit, typesRemove, history } = props;

  if (listKbcgwTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listKbcgwTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...
    loading: listKbcgwTypeQuery.loading,
    remove,
    renderButton
  };
