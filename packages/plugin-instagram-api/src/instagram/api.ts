...
export class instagramAPI {
  ...
  constructor(config: any) {
    ...
    this.headers = {
      'X-instagram-Auth-Token': config.token
    };
  }
  ...
  async registerWebhook(): Promise<any> {
    ...
    const url: string = `${domain}/pl:instagram/webhook/${this.integrationId}`;

    const payload: IRequestParams = {
      method: 'POST',
      headers: this.headers,
      url: 'https://chatapi.instagram.com/pa/set_webhook',
      body: {
        url,
        event_types: [
          'delivered',
  ...
  async sendMessage(message): Promise<any> {
    ...
    const requestPayload: IRequestParams = {
      method: 'POST',
      headers: this.headers,
      url: 'https://chatapi.instagram.com/pa/send_message',
      body: {
