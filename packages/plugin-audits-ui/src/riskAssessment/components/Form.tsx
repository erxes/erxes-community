...
class Form extends React.Component<Props & ICommonFormProps, State> {
  ...
  renderLogic({ _id, name, logic, value, value2, color }: RiskCalculateLogicType, formProps) {
    const handleRow = e => {
      ...
      const newVariables =
        ...
        riskAssessment?.calculateLogics.map(logic => (logic._id === _id ? { ...logic, [name]: ['value', 'value2'].includes(name) ? parseInt(value) : value } : logic));
      this.setState(prev => ({
    ...
    const removeLogicRow = e => {
      const { riskAssessment } = this.state;
      const removedLogicRows = riskAssessment.calculateLogics && riskAssessment?.calculateLogics.filter(logic => logic._id !== _id);
      this.setState(prev => ({
    ...
    const onChangeColor = hex => {
      const { riskAssessment } = this.state;
      const newVariables = riskAssessment.calculateLogics && riskAssessment.calculateLogics.map(logic => (logic._id === _id ? { ...logic, color: hex } : logic));
      this.setState(prev => ({
        riskAssessment: { ...prev.riskAssessment, calculateLogics: newVariables }
      }));
    };
    ...
    const renderColorSelect = selectedColor => {
      const popoverBottom = (
        <Popover id="color-picker">
          <TwitterPicker width="266px" triangle="hide" color={selectedColor} onChange={e => onChangeColor(e.hex)} colors={COLORS} />
        </Popover>
      );

      return (
        <OverlayTrigger trigger="click" rootClose={true} placement="bottom-start" overlay={popoverBottom}>
          <ColorPick>
    ...
    return (
      <FormWrapper style={{ margin: '5px 0' }} key={_id}>
        <FormColumn>
          <FormControl {...formProps} name="name" type="text" defaultValue={name} onChange={handleRow} required />
        </FormColumn>
        <FormColumn>
          <FormControl name="logic" {...formProps} componentClass="select" required defaultValue={logic} onChange={handleRow}>
            <option />
        ...
        <FormColumn>
          <FormContainer row gap align="center">
            <FormControl {...formProps} name="value" type="number" defaultValue={value} onChange={handleRow} required />
            {logic === '(≈) between' && (
              <>
                <span>-</span>
                <FormControl {...formProps} name="value2" type="number" defaultValue={value2} onChange={handleRow} required />
              </>
            )}
          </FormContainer>
        </FormColumn>
        ...
        <Tip text="Remove Row" placement="bottom">
          <Button btnStyle="danger" icon="times" onClick={removeLogicRow} style={{ marginLeft: '10px' }} />
        </Tip>
      </FormWrapper>
    );
  }
  ...
  renderLogics(formProps) {
    ...
    return riskAssessment.calculateLogics && riskAssessment.calculateLogics.map(logic => this.renderLogic(logic, formProps));
  }
    ...
    return (
      <>
        <FormGroup>
          ...
          <FormControl {...formProps} key="name" name="name" type="text" required={true} value={riskAssessment.name} onChange={handleState} />
        </FormGroup>
        <FormGroup>
          <ControlLabel>{__('Description')}</ControlLabel>
          <FormControl {...formProps} name="description" componentClass="textarea" value={riskAssessment.description} onChange={handleState} />
        </FormGroup>
        <FormGroup>
          <ControlLabel>{__('Category')}</ControlLabel>
          <SelectWithCategory name="categoryId" label="Choose Category" multi={false} initialValue={!categoryId ? riskAssessment.categoryId : categoryId} onSelect={handleChangeCategory} />
        </FormGroup>
        <FormGroup>
          <ControlLabel>{__('Calculate Methods')}</ControlLabel>
          <Select placeholder={__('Select Calculate Method')} value={riskAssessment?.calculateMethod} options={calculateMethods} multi={false} onChange={handleChangeCalculateMethod} />
        </FormGroup>
        <FormWrapper>
  ...
  render() {
    ...
    return (
      <CommonForm {...this.props} renderContent={this.renderContent} generateDoc={this.generateDoc} object={this.props.object} renderButton={renderBtn()} createdAt={assessmentDetail?.createdAt} />
    );
  }
}
