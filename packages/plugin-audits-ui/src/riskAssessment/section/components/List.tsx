...
import RiskAssessmentForm from '../containers/Form';
...
function RiskAssessmentSection(props: Props) {
  ...
  const renderFormModalContent = ({ closeModal, riskAssessmentId }: { closeModal: () => void; cardId?: string; riskAssessmentId?: string }) => {
    return <RiskAssessmentForm {...props} closeModal={closeModal} riskAssessmentId={riskAssessmentId} />;
  };

  const renderFormModal = (trigger: React.ReactNode, riskAssessmentId?: string) => {
    return <ModalTrigger size="lg" trigger={trigger} content={props => renderFormModalContent({ ...props, riskAssessmentId })} title="Risk Assessment" />;
  };
  ...
  const renderSubmissionForm = (isSubmitted?: boolean, riskAssessmentId?: string) => {
    const trigger = (
      <Button btnStyle="link">
        <Tip text={isSubmitted ? 'See Submitted Form' : 'Submission Form'}>
          <Icon color={isSubmitted ? 'purple' : 'green'} icon={isSubmitted ? 'file-edit-alt' : 'file-alt'} />
        </Tip>
      </Button>
    );
    ...
    return <ModalTrigger title="Your Risk Assessment Submission Form" trigger={trigger} content={content} backDrop isAnimate />;
  };
    ...
    <>
      ...
      >
        {list.length ? (
          <div>
            {list.map(item => (
              <SectionBodyItem key={item.riskAssessmentId}>{renderFormModal(renderItem(item.riskAssessment), item.riskAssessmentId)}</SectionBodyItem>
            ))}
          </div>
        ) : (
          ...
          {submissions ? (
            submissions.map(user => (
              <SectionBodyItem key={user._id}>
                <ProductName>
                  {user.email}
                  {currentUser.email === user.email && renderSubmissionForm(user.isSubmittedRiskAssessmentForm, list[0].riskAssessmentId)}
                </ProductName>
              </SectionBodyItem>
            ))
          ) : (
