...
import { ICardRiskAssessmentDetailQueryResponse, ICardRiskAssessmentsQueryResponse } from '../../common/types';
...
class RiskAssessmentForm extends React.Component<FinalProps, State> {
  ...
  renderAddForm(props) {
    ...
    return <AddRiskAssessmentForm {...props} renderButton={renderButton} generateDoc={generateDoc} />;
  }

  handleSelect = item => {
    const { id, refetch, addConformity, conformityDetail, editconformity, removeConformity, refetchSubmissions } = this.props;
    ...
    if (conformityDetail.riskConformityDetails && conformityDetail.riskConformityDetails.length > 0) {
