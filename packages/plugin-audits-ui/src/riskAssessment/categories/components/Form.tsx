...
import { ContentWrapper, FormContainer, PreviewWrapper, Box as FormTriggerBtn, Typography } from '../../../styles';
    ...
    const formTrigger = (
      <FormTriggerBtn selected={detail || doc.formId ? true : false}>
        <FormContainer row justifyCenter gapBetween={10}>
          <Icon size={15} icon={detail ? 'file-edit-alt' : 'file-plus-alt'} color={colors.colorPrimary} />
    ...
    return (
      <>
        ...
        <CustomFormGroup label={detail ? 'Edit a form' : 'Build a form'}>
          <FormContainer>
            <ModalTrigger isAnimate title="Build New Form" enforceFocus={false} size="xl" content={renderFormContent} trigger={formTrigger} />
          </FormContainer>
        </CustomFormGroup>
        ...
        <ModalFooter>
          <Button btnStyle="simple" icon="times-circle" uppercase={false} onClick={this.handleCloseForm}>
            Cancel
          </Button>
