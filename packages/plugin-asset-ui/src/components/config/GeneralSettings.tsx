...
class GeneralSettings extends React.Component<Props, State> {
  constructor(props: Props) {
    ...
    this.state = {
      ...
      default_uom: props.configsMap.default_uom ? props.configsMap.default_uom : ''
    };
  }
  ...
  render() {
    const breadcrumb = [
      ...
      { title: __('Assets config') }
    ];

    const actionButtons = (
      <Button btnStyle="primary" onClick={this.save} icon="check-circle" uppercase={false}>
        Save
      </Button>
    );
    ...
    const content = (
      <ContentBox id={'GeneralSettingsMenu'}>
        <CollapseContent title="General settings">
          ...
          {this.state.is_uom && this.renderCombobox('default_uom', 'default uom')}
        </CollapseContent>
      </ContentBox>
    );
    ...
    return (
      <Wrapper
        header={<Wrapper.Header title={__('Products config')} breadcrumb={breadcrumb} />}
        mainHead={<Header />}
        actionBar={
          <Wrapper.ActionBar
