...
import { ACTIVITY_LOG_ACTIONS, putActivityLog, putUpdateLog } from '../../logUtils';
...
const sendConversationToIntegrations = async (
  ...
  if (type === 'facebook') {
    ...
    try {
      await messageBroker().sendRPCMessage(RABBITMQ_QUEUES.RPC_API_TO_INTEGRATIONS, {
        action,
        type,
...
const conversationMutations = {
  ...
  async conversationMessageAdd(_root, doc: IConversationMessageAdd, { user, dataSources }: IContext) {
    ...
    if (
       ...
       */
      const chunks = doc.content.length > 160 ? splitStr(doc.content, 160) : [doc.content];
  ...
  async conversationsChangeStatusFacebookComment(_root, doc: IReplyFacebookComment, { dataSources }: IContext) {
    ...
    return sendConversationToIntegrations(type, '', conversationId, requestName, doc, dataSources, action);
  },
  ...
  async conversationsUnassign(_root, { _ids }: { _ids: string[] }, { user }: IContext) {
    const { oldConversations, oldConversationById } = await getConversationById({ _id: { $in: _ids } });
  ...
  async conversationConvertToCard(_root, params: IConversationConvert, { user, docModifier }: IContext) {
    ...
    if (itemId) {
      ...
      if (bookingProductId) {
        const { product, dealUOM, dealCurrency } = await checkBookingConvert(bookingProductId);
  ...
  async conversationEditCustomFields(_root, { _id, customFieldsData }: { _id: string; customFieldsData: any }) {
