...
import { ISendMessageArgs, paginate, sendMessage } from '@erxes/api-utils/src/core';
...
export const initBroker = cl => {
  ...
  consumeRPCQueue('inbox:createConversationAndMessage', async ({ subdomain, data }) => {
  ...
  consumeRPCQueue(
    ...
    async ({ subdomain, data: { conversationId, content, userId, customerId } }) => {
      const models = await generateModels(subdomain);
  ...
  consumeQueue('inbox:integrationsNotification', async ({ subdomain, data }) => {
  ...
  consumeRPCQueue('inbox:getConversation', async ({ subdomain, data: { conversationId } }) => {
    const models = await generateModels(subdomain);
  ...
  consumeRPCQueue('inbox:conversations.findOne', async ({ subdomain, data: { query } }) => {
    const models = await generateModels(subdomain);
  ...
  consumeRPCQueue('inbox:conversations.count', async ({ subdomain, data: { query } }) => {
    const models = await generateModels(subdomain);
  ...
  consumeRPCQueue('inbox:getConversationsList', async ({ subdomain, data: { query, listParams } }) => {
    const models = await generateModels(subdomain);
  ...
  consumeRPCQueue('inbox:getModuleRelation', async ({ data: { module, target } }) => {
    let filter;
  ...
  consumeQueue('inbox:conversationClientMessageInserted', async ({ subdomain, data }) => {
    const models = await generateModels(subdomain);
...
export const sendCommonMessage = async (args: ISendMessageArgs & { serviceName: string }): Promise<any> => {
  return sendMessage({
    serviceDiscovery,
    client,
    ...args,
...
export const sendMessageNotification = async (data?: any): Promise<any> => {
