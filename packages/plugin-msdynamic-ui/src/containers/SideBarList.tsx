...
const TypesListContainer = (props: FinalProps) => {
  const { listMsdynamicTypeQuery, typesEdit, typesRemove, history } = props;

  if (listMsdynamicTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listMsdynamicTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...
    loading: listMsdynamicTypeQuery.loading,
    remove,
    renderButton
  };
