...
import Queries, { IParam } from './graphql/resolvers/queries/performs';
...
const generateParams = ({ queryParams }) => ({
  ...
  vendorIds: queryParams.vendorIds && queryParams.vendorIds.split(','),
  productIds: queryParams.productIds && queryParams.productIds.split(','),
  jobReferId: queryParams.jobReferId && queryParams.jobReferId.split(',')
});
...
const prepareData = async (
  ...
  const params: IParam = generateParams({ queryParams: query });
  const perPage = 20;

  const count = await Queries.performsCount({}, params, {
    ...({ subdomain, models, commonQuerySelector: {} } as any)
  });
  ...
  for (let page = 1; page <= pageCount; page++) {
    const performs = await Queries.performs(
      {},
      { ...params, perPage, page },
      { ...({ subdomain, models, commonQuerySelector: {} } as any) }
    );
    mainDatas = mainDatas.concat(performs);
  }
  return mainDatas;
};
...
export const buildFile = async (
  ...
  const mainData = await prepareData(models, subdomain, query);
  ...
  for (const perform of mainData) {
    rowIndex++;

    const fullItem = {
      ...perform,
      inProducts: undefined,
      outProducts: undefined
    };

    // Iterating through basic info columns
    for (const column of headers) {
      for (const item of perform.inProducts) {
        fullItem.inProductItem = item;
        fullItem.outProductItem = {};
        const cellValue = await fillCellValue(column.name, fullItem);

        addCell(column, cellValue, sheet, columnNames, rowIndex);
      }

      for (const item of perform.outProducts) {
        fullItem.inProductItem = '';
        fullItem.outProductItem = item;
        const cellValue = await fillCellValue(column.name, fullItem);
