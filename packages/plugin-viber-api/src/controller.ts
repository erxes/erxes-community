...
const init = async (app: any) => {
  ...
  app.post('/set_webhook', async (req: any, res: any) => {
    const response = await setWebhook(req.body.webhook_url);
    return res.json(response);
  });

  app.post('/remove_webhook', async (req: any, res: any) => {
  ...
  app.post('/broadcast_message', async (req: any, res: any) => {
    const response = await broadcastMessage();
    return res.json(response);
  });

  app.post('/get_account_info', async (req: any, res: any) => {
    const response = await getAccountInfo();
    return res.json(response);
  });

  app.post('/get_user_detail', async (req: any, res: any) => {
    const response = await getUserDetail(req.body.id);
    return res.json(response);
  });

  app.post('/get_online', async (req: any, res: any) => {
